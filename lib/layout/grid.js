   ...
   for (var i=0; i<this.options.rows; i++) {
      for (var j=0; j<this.options.cols; j++) {
         ...
         if (this.matrix[i][j].obj instanceof Grid) {
            ...
            grid.applyLayout(screen, {x: left, y: top, width: (width * this.matrix[i][j].colSpan), height: (height * this.matrix[i][j].rowSpan)})
         }
         else {
            this.matrix[i][j].opts.top = top + "%"
            this.matrix[i][j].opts.left = left + "%"
            this.matrix[i][j].opts.width = (width * this.matrix[i][j].colSpan) + "%"
            this.matrix[i][j].opts.height = (height * this.matrix[i][j].rowSpan) + "%"
