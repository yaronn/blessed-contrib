...
Line.prototype.setData = function(data) {           
    ...
    function formatYLabel(value, max, numLabels) {      
      var fixed = (max/numLabels<1 && value!=0) ? 2 : 0
      return value.toFixed(fixed)
    }

    function getMaxXLabelPadding(numLabels) {
      var max = getMaxY()

      return formatYLabel(max, max, numLabels).length * 2;
    }

    if (getMaxXLabelPadding(this.options.numYLabels) > xLabelPadding) {
    ...
    function getYPixel(val) { 
        ...
        res-=3 //to separate the baseline and the data line to separate chars so canvas will show separate colors
        return res
    }
    ...
    for(var i = 0; i < maxY; i += yLabelIncrement) {    
        c.fillText(formatYLabel(i, maxY, this.options.numYLabels), xPadding - xLabelPadding, getYPixel(i));    
    }
