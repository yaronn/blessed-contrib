...
Line.prototype.setData = function(data) {
  ...
  function getMinY() {
    ...
    for(let i = 0; i < data.length; i++) {
      if (data[i].y.length) {
        var current = _.min(data[i].y, parseFloat);
        if (current < min) {
          min = current;
        }
      }
  ...
  this.options.minY = getMinY();

  function formatYLabel(value, max, min, numLabels, wholeNumbersOnly, abbreviate) {
    var fixed = (((Math.abs(max) - Math.abs(min)) / numLabels) < 1 && value!=0 && !wholeNumbersOnly) ? 2 : 0;
