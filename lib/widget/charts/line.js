...
Line.prototype.setData = function(data) {           
    ...
    function formatYLabel(value, max) {
      return value.toFixed(max<1?2:0)
    }

    function getMaxXLabelPadding() {
      var max = getMaxY()

      return formatYLabel(max, max).length * 2;
    }
    ...
    for(var i = 0; i < maxY; i += yLabelIncrement) {    
        c.fillText(formatYLabel(i, maxY), xPadding - xLabelPadding, getYPixel(i));    
    }
